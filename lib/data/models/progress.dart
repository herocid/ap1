...
enum SessionMode {
  /// Freies Ueben mit Sofort-Feedback.
  uebung,

  /// Gezieltes Training einer Schwachstelle (aus dem Dashboard heraus).
  fokus,

  /// Pruefungssimulation: Zeitdruck, kein Feedback waehrend des Laufs.
  pruefung,
}

/// Ein beantwortetes Item. Die gesamte Statistik der App laesst sich aus
...
class TopicStat {
  ...
  /// 0..1 - gewichtete Trefferquote, juengere Antworten zaehlen mehr.
  final double mastery;

  /// 0..1 - wie viel vom Thema ueberhaupt schon angefasst wurde.
  final double coverage;
  final DateTime? lastSeen;

  /// Das, was im Dashboard als Balken erscheint: Koennen *und* Abdeckung.
  double get confidence => mastery * (0.45 + 0.55 * coverage);
}

/// Badges sind bewusst an Verhalten gekoppelt, das wirklich zum Bestehen
/// beitraegt (Dranbleiben, Schwachstellen angehen, Simulation ueberstehen) -
/// nicht an reine Nutzungsdauer.
enum Achievement {
  ersterTag('Erster Schritt', 'Erste Session abgeschlossen'),
  woche('Sieben am Stueck', '7 Tage Streak'),
  netzplanProfi('Netzplan-Profi', '10 Netzplan-Aufgaben fehlerfrei'),
  fehlerjaeger('Fehlerjaeger', '20 frueher falsche Aufgaben korrigiert'),
  simulant('Ernstfall bestanden', 'Pruefungssimulation mit >= 50 % beendet'),
...
class ProgressState {
  ...
  /// fuers Versuchen. Kein XP-Multiplikator fuer Tempo - das wuerde zu
  /// Ratewuerfeln erziehen.
  ...
  /// korrigiert wurden - die Grundlage fuer den Fehlerspeicher.
  ...
  /// Statistik je Thema. [poolSize] liefert die Gesamtzahl verfuegbarer
  ...
  Map<String, TopicStat> topicStats(Map<String, int> poolSize) {
    ...
    for (final t in Topics.all) {
      ...
      // Exponentielle Gewichtung: die letzten Antworten zaehlen mehr als die
  ...
  /// Der Pruefungsreife-Indikator (0..100).
  ///
  /// Gewichteter Mittelwert der Themen-Confidence, gewichtet mit dem
  /// geschaetzten Punkteanteil des Themas in der AP1. Themen, die noch gar
  /// nicht angefasst wurden, zaehlen als 0 - der Wert soll nicht dadurch
  ...
  String readinessLabel(int value) {
    if (value >= 80) return 'Pruefungsreif';
